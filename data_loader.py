...
def load_data_range(
    ...
    # Find all CSV files in the Fitbit folder and subfolders
    ...
    for date in date_range:
        ...
        # Try to find a heart rate file for the current date
        for f in all_csv_files:
            if f"heart_rate_{current_date_str}.csv" in os.path.basename(f):
                hr_df = pd.read_csv(f)
        ...
        # Try to find a steps file for the current month
        steps_month_str = date.strftime("%Y-%m-01")
        for f in all_csv_files:
            if f"steps_{steps_month_str}.csv" in os.path.basename(f):
                monthly_steps_df = pd.read_csv(f)
